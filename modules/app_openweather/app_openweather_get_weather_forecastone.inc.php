<?php
while($ret<=3) {
  $url="http://api.openweathermap.org/data/2.5/onecall?lat=" . $cities[$i]['CITY_LAT'] ."&lon=" . $cities[$i]['CITY_LON'] . "&exclude=current,minutely,hourly&mode=json&units=" . $unit . "&lang=" . $lang . "&appid=" . $apiKey;
  if($this->config['debug_level']>=2) debmes('[DBG] --- [forecast one] '.$url, 'openweather');
  $data =  getURL($url);
  if($this->config['debug_level']>=2) debmes('[DBG] +++ [forecast one] '.$data, 'openweather');
  ...
  if ($weather->daily) {
    ...
    foreach($weather->daily as $period) {
      ...
      if (stripos($cities[$i]['EXCLUDE_PRP'], 'wind_speed')===false)            sg($obj.'.wind_speed', round($period->wind_speed, $round));
      //other
      if (stripos($cities[$i]['EXCLUDE_PRP'], 'humidity')===false &&
      $period->humidity)                                                        sg($obj.'.humidity', round($period->humidity, $round));
      if (stripos($cities[$i]['EXCLUDE_PRP'], 'dew_point')===false)             sg($obj.'.dew_point', round($period->dew_point, $round));
      ...
      if (stripos($cities[$i]['EXCLUDE_PRP'], 'uvindex')===false)               sg($obj.'.uvindex', round($period->uvi, $round));
      if (stripos($cities[$i]['EXCLUDE_PRP'], 'visibility')===false)            sg($obj.'.visibility', $period->visibility);
      //weather
      ...
      //system
      //if (stripos($cities[$i]['EXCLUDE_PRP'], 'sunrise')===false)               sg($obj.'.sunrise', $period->sunrise);
      //if (stripos($cities[$i]['EXCLUDE_PRP'], 'sunset')===false)                sg($obj.'.sunrise', $period->sunset);
      //sun info
